...
{
    public class AvatarBundleRepository(ServiceFacade.Repository repositoryFacade,
        ILogger<AvatarBundleRepository> logger)
            : AuditingRepository<Model.AvatarBundle, AvatarBundle>(repositoryFacade, logger),
            IAvatarBundleRepository
    {
        ...
        public async Task<ICollection<AvatarBundleTransfer>> GetForExportAsync(int siteId)
        {
            var forkedConfig = _mapper.Config
                .Fork(_ => _.NewConfig<Model.AvatarBundle, AvatarBundleTransfer>()
                    .Map(d => d.AvatarItems, s => s.AvatarBundleItems.Select(_ => _.AvatarItem)));

            return await DbSet.AsNoTracking()
                .Where(_ => _.SiteId == siteId)
                .ProjectToType<AvatarBundleTransfer>(forkedConfig)
                .ToListAsync();
        }
